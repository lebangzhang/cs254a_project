...
void kernel_body(kernel_arg_t* __UNIFORM__ arg) {
    ...
    uint32_t tid   = blockIdx.x;   // VX: one thread per block
    uint32_t index = tid * 2;      // x2 because each Node has 2 elements

    if ((tid < num_nodes) && (mask[tid])) {
        mask[tid] = 0;  // consume current frontier node
        ...
        for (int i = start; i < end; i++) {
            ...
            // only discover unvisited neighbors
            if (!visit[nid]) {
                visit[nid]   = 1;               // mark as visited right away
                cost[nid]    = cost[tid] + 1;   // distance update
                update[nid]  = 1;               // add to next frontier
                thread_update[tid] = 1;         // indicate progress
            }
        }
    }
}

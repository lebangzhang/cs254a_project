...
void generate_random_graph(int num_nodes, int max_edges_per_node,
    ...
    // --- Step 1: Create a chain to guarantee connectivity ---
    for (int i = 0; i < num_nodes; ++i) {
        nodes[i].starting = edge_count;
        ...
        if (i < num_nodes - 1) {
            ...
            edges.push_back(i + 1);
            edge_count++;
            nodes[i].no_of_edges = 1;
        } else {
            // last node: start with 0 outgoing edges (we will add random extras next)
            nodes[i].no_of_edges = 0;
        }
    }

    // --- Step 2: Add extra random edges up to max_edges_per_node ---
    for (int i = 0; i < num_nodes; ++i) {
        int existing_edges = nodes[i].no_of_edges;
        int extra_edges = (rand() % (max_edges_per_node + 1 - existing_edges)); // avoid exceeding max
        for (int e = 0; e < extra_edges; ++e) {
            int dest_node = rand() % num_nodes;
            if (dest_node != i) {  // avoid self-loop
                edges.push_back(dest_node);
                nodes[i].no_of_edges++;
                edge_count++;
            }
...
/*  int h_edges[12] = { 2,7,2,5,1,4,4,6,4,1,5, 6};*/
  ...
  int continue_flag = 1;
  std::vector<int> h_dst_src5(num_nodes);
  std::vector<int> h_dst_src4(num_nodes);
  std::vector<int> h_dst_src3(num_nodes);
  std::vector<int> h_dst_src2(num_nodes);

  uint64_t total_cycles_per_core = 0;
  uint64_t cycles_per_core = 0;
  uint64_t total_instrs_per_core = 0;
  uint64_t instrs_per_core = 0;
  ...
  while(continue_flag){
    ...
    RT_CHECK(vx_copy_from_dev(h_dst_src5.data(), src5_buffer, 0, cost_buf_size));
    ...
    // Next Frontier
    std::cout << "download destination buffer" << std::endl;
    RT_CHECK(vx_copy_from_dev(h_dst_src2.data(), src2_buffer, 0, cost_buf_size));
    std::cout << "download destination buffer" << std::endl;
    RT_CHECK(vx_copy_from_dev(h_dst_src3.data(), src3_buffer, 0, cost_buf_size));
        std::cout << "download destination buffer" << std::endl;
    RT_CHECK(vx_copy_from_dev(h_dst_src4.data(), src4_buffer, 0, cost_buf_size));

    for (uint32_t tid = 0; tid < num_nodes; tid++) {
        if (h_dst_src3[tid]) {
            // promote newly discovered nodes into the next frontier
            h_dst_src2[tid] = 1;   // mask: next frontier
            h_dst_src3[tid] = 0;   // clear update after consuming it
        } else {
            h_dst_src2[tid] = 0;   // no frontier for this node
        }
    ...
    // upload source buffer2 (Mask)
    std::cout << "upload source buffer2" << std::endl;
    RT_CHECK(vx_copy_to_dev(src2_buffer, h_dst_src2.data(), 0, nodes_buf_size));
    // upload source buffer3 (Update)
    std::cout << "upload source buffer3" << std::endl;
    RT_CHECK(vx_copy_to_dev(src3_buffer, h_dst_src3.data(), 0, nodes_buf_size));
    // upload source buffer4 (Visit)
    std::cout << "upload source buffer4" << std::endl;
    RT_CHECK(vx_copy_to_dev(src4_buffer, h_dst_src4.data(), 0, nodes_buf_size));
    // upload source buffer5 (Thread Update)
    std::cout << "upload source buffer5" << std::endl;
    RT_CHECK(vx_copy_to_dev(src5_buffer, h_dst_src5.data(), 0, nodes_buf_size));
  }  

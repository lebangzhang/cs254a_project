...
int main(int argc, char *argv[]) {
  ...
  // Transpose A on host: At[j*n+i] = A[i*n+j] for coalesced GPU access
  std::vector<TYPE> h_At(size * size);
  for (uint32_t r = 0; r < size; ++r)
    for (uint32_t c = 0; c < size; ++c)
      h_At[c * size + r] = h_src0[r * size + c];

  // upload source buffer0 (transposed A)
  std::cout << "upload source buffer0 (transposed A)" << std::endl;
  RT_CHECK(vx_copy_to_dev(src0_buffer, h_At.data(), 0, A_buf_size));

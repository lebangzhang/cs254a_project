...
int main(int argc, char *argv[]) {
  ...
  uint32_t buf_size_sel = num_points * sizeof(uint32_t);
  ...
  kernel_arg.num_threads_to_run = num_points/vec_len_per_thread;

  kernel_arg.multiplier = 1.0 / (1.0 - dropout_p);
  ...
  for (uint32_t i = 0; i < num_points; ++i) {
    ...
    uint32_t sel_bit = h_src1[i] & 0x1; 

    auto ref = (sel_bit) ? 0 : kernel_arg.multiplier * h_src0[i];
    auto cur = h_dst[i];

...
vx_buffer_h B_buffer    = nullptr;  // original B (kept for cleanup)
vx_buffer_h Bt_buffer   = nullptr;  // transposed B (used by kernel)
...
static void show_usage() {
  std::cout << "Vortex sgemm Test." << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  // B_buffer holds original B — not passed to kernel
  RT_CHECK(vx_mem_alloc(device, buf_size, VX_MEM_READ,  &Bt_buffer));
  RT_CHECK(vx_mem_address(Bt_buffer, &kernel_arg.B_addr));  // kernel reads Bt
  RT_CHECK(vx_mem_alloc(device, buf_size, VX_MEM_WRITE, &C_buffer));
  RT_CHECK(vx_mem_address(C_buffer,  &kernel_arg.C_addr));

  std::cout << "A_addr=0x"  << std::hex << kernel_arg.A_addr << std::endl;
  std::cout << "Bt_addr=0x" << std::hex << kernel_arg.B_addr << std::endl;
  std::cout << "C_addr=0x"  << std::hex << kernel_arg.C_addr << std::endl;

  std::vector<TYPE> h_A(size_sq), h_B(size_sq), h_Bt(size_sq), h_C(size_sq);
  ...
  for (uint32_t r = 0; r < size; ++r)
    for (uint32_t c = 0; c < size; ++c)
      h_Bt[c * size + r] = h_B[r * size + c];

  std::cout << "upload matrix A buffer"  << std::endl;
  RT_CHECK(vx_copy_to_dev(A_buffer,  h_A.data(),  0, buf_size));
  std::cout << "upload matrix Bt buffer" << std::endl;
  RT_CHECK(vx_copy_to_dev(Bt_buffer, h_Bt.data(), 0, buf_size));
  ...
  printf("Elapsed time: %lg ms\n",
    std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start).count() * 1.0);

...
int main(int argc, char *argv[]) {
  ...
  // One logical thread per RVV chunk — block_dim must be 1 so that
  // (blockIdx.x * blockDim.x + threadIdx.x) == blockIdx.x, giving each
  // thread a unique non-overlapping index into the element array.

...
static void show_usage() {
  std::cout << "Vortex jacobi_v Test." << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::vector<TYPE> h_src1(size, 0.0f);
  std::vector<TYPE> h_src2(size);
  std::vector<TYPE> h_dst(size);

  for (uint32_t i = 0; i < size * size; ++i) h_src0[i] = Comparator<TYPE>::generate();
  for (uint32_t i = 0; i < size; ++i)        h_src2[i] = Comparator<TYPE>::generate();
  ...
  uint64_t cycles_per_core, instrs_per_core;
  ...
  for (uint32_t k = 0; k < iteration; k++) {
    ...
    // Correct vx_mpm_query signature: (device, core, csr, warp, result)
    ...
    // Feed output back as x_old for next iteration
    for (uint32_t i = 0; i < size; ++i) h_src1[i] = h_dst[i];
    std::cout << "upload source buffer1" << std::endl;
    RT_CHECK(vx_copy_to_dev(src1_buffer, h_src1.data(), 0, dst_buf_size));
  ...
  std::vector<TYPE> h_gold(size, 0.0f);
  std::vector<TYPE> h_result(size);
  jacobi_cpu(h_src0.data(), h_gold.data(), h_result.data(), h_src2.data(), size, (int)iteration);

  int errors = 0;
  for (uint32_t i = 0; i < size; ++i) {
    if (!Comparator<TYPE>::compare(h_dst[i], h_result[i], i, errors))
      ++errors;
    printf("%f\n", h_dst[i]);
  }

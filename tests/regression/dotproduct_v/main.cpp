...
static void show_usage() {
  std::cout << "Vortex Dotproduct_V Test." << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  // Grid: one logical thread per RVV chunk — matches dotproduct's vx_max_occupancy_grid pattern
  // but we divide num_points by vec_len_per_thread so each thread owns exactly one chunk.
  uint32_t num_threads = num_points / vec_len_per_thread;
  uint32_t grid_dim[1], block_dim[1];
  RT_CHECK(vx_max_occupancy_grid(device, 1, &num_threads, grid_dim, block_dim));

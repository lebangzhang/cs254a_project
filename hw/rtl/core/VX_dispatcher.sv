...
`endif
    ...
    // LSU path: dedicated slice computes effective base address for
    // pack-loads / RVV unit/strided/indexed segment ops, applies v0
    // mask predication, and drives the LSU dispatch buffer.
    VX_lsu_dispatch #(
        ...
        .operand_data    (operands_if.data),
        .ready_in        (operands_ready_in[EX_LSU]),
        .dispatch_if_lsu (dispatch_if[EX_LSU])
    );

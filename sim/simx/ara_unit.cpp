...
class AraUnit::Impl {
public:
  Impl(AraUnit* simobject, const Arch& arch, Core* core)
...
bool AraUnit::set_csr(uint32_t addr, uint32_t wid, uint32_t tid, Word value) {
  return impl_->set_csr(addr, wid, tid, value);
}

void AraUnit::load(const Instr &instr, uint32_t wid, uint32_t tid, const std::vector<reg_data_t>& rs1_data, const std::vector<reg_data_t>& rs2_data, MemTraceData* trace_data) {
  return impl_->load(instr, wid, tid, rs1_data, rs2_data, trace_data);
}

void AraUnit::store(const Instr &instr, uint32_t wid, uint32_t tid, const std::vector<reg_data_t>& rs1_data, const std::vector<reg_data_t>& rs2_data, MemTraceData* trace_data) {
  return impl_->store(instr, wid, tid, rs1_data, rs2_data, trace_data);
}

AraUnit::ExeRet AraUnit::execute(const Instr &instr, uint32_t wid, uint32_t tid, const std::vector<reg_data_t>& rs1_data, const std::vector<reg_data_t>& rs2_data, std::vector<reg_data_t>& rd_data, ExeTraceData* trace_data) {
  return impl_->execute(instr, wid, tid, rs1_data, rs2_data, rd_data, trace_data);
}

const AraUnit::PerfStats& AraUnit::perf_stats() const {
